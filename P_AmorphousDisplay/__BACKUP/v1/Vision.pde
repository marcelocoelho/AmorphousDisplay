...
void loadCameraViews() {
	...
	image( opencv.image(OpenCV.MEMORY), visionX, visionY+244, 160, 120 );			// left IMAGE STORED IN MEMORY
	
	
	opencv.absDiff();
	opencv.threshold(vThreshold);
	
	image( opencv.image(), 852, visionY+244, 160, 120 );						// right BUFFERED OPENCV.IMAGE()
