...
public class Canvas {
	...
	void loadMovieSequence(String _folderName) {
		...
		// write bit of code here to load each one of the images in folder I want
		/*
		for (int i = 0; i < numFrames; i++) {
			allFrames.get(i).loadMovieFromDirectory(_folderName+"/frame"+(i+1)+".png");
		}
		*/
	}
	...
	/*
	void saveMovie(String _filename) {
		I couldn't get this to work from here
	}*/
	...
	void loadAlpha(PImage _alphaImage) {
		...
		alphaImage.filter(POSTERIZE, 10);  // 10 is numebr of frames		
		alphaImage.endDraw();
		...
		for (int i = 0; i < numFrames; i++) {
			...
			for (int j = 0; j < tempImage.pixels.length; j++) {
				...
				if (red(tempImage.pixels[j]) > int(boundary)+2 || red(tempImage.pixels[j]) < int(boundary)-2) {  // 2 here is just a margin of safety
			     tempImage.pixels[j] = color(255,255,255,255);
			   } else {
